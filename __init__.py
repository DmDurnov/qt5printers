...
class DumperBase():
    @staticmethod
    def warn(message):
        print(message) # simplified - no replacements
    ...
    def qtVersion(self):
        ...
        # simplified - no qtversionAtLeast6
        return 0x060602
    ...
    # added this convenience function
    def qt6orLater(self):
        return self.qtVersionAtLeast(0x060000)

    def lookupType(self, typename):
        # simplified - must be overridden in bridges
        pass
    ...
    def qArrayDataHelper(self, array_data_ptr):
        ...
        if self.qtVersionAtLeast(0x050000):
            ...
            # removed Qt 4 and earlier support
            warn("unsupported Qt version < 5.0")
        return data, length, alloc
    ...
    # simplified - removed almost everything
    class Value():
    ...
    # commented out all val.typeid assignments,
    ...
    def createValueFromData(self, data, typish):
        ...
        val.ldata = data
        # commented out, because we have not copied Value.check() for simplicity
        #val.check()
        return val
    ...
    def describeStruct(self, pattern):
        ...
        n = None
        # removed support for the `{type}` syntax in the pattern of split()
...
class Dumper(DumperBase):
    ...
    def __init__(self):
        ...
        # the following two non-comment lines were copied verbatim
    ...
    def extractQtVersion(self):
        try:
            ...
            # we do not care about Qt 5.1 and Qt 5.2, so just return 5.0 as fallback
            (major, minor, patch) = (5, 0, 0)
            return 0x10000 * major + 0x100 * minor + patch

    # added this method to override the stub in DumperBase
    def lookupType(self, typename):
        return gdb.lookup_type(typename)


# the bodies of the functions parseQCborContainer() and qCborContainerBytedata()
...
# removed unneeded parameter `is_bytes`
def qdumpHelper_QCbor_string(d, container_ptr, element_index):
    # extracted the helper functions parseQCborContainer()
    ...
    # return the values instead of dumping them
...
def qdumpHelper_QCborArray_valueAt(d, container_ptr, elements_data_ptr, idx, bytedata, is_cbor):
    ...
    # return a tuple instead of wrapping the values in d.createProxyValue()
...
class QMapPrinter(PrinterBaseType):
    ...
    class _iteratorQt5:
        ...
        def __next__(self):
            if not self.moveToNextNode():
                raise StopIteration
            self.current_typed = self.current.cast(self.node_p_type)
    ...
    def display_hint (self):
        return None # HACK: print command supposed to display_hint
...
class CborOrJsonValueData:
    ...
    def toGdbValueString(self, element_index, is_bytes):
        bytedata_data, bytedata_len, element_flags = qdumpHelper_QCbor_string(
...
def qCborContainerValueAt(container_ptr, elements_data_ptr, idx, bytedata, is_cbor):
    return CborOrJsonValueData(*qdumpHelper_QCborArray_valueAt(
...
class QCborContainerPrivateIterator:
    def __init__(self, container_ptr, containerClassName, childName = None):
        ...
        self.data_pos, self.elements_data_ptr, self.size = parseQCborContainer(dumper, container_ptr)
        self.bytedata = None
        self.index = 0
    ...
    def valueAt(self, index):
        if self.bytedata is None:
            self.bytedata = qCborContainerBytedata(dumper, self.data_pos)
...
class QCborContainerPrinterBase(PrinterBaseType):
    def __init__(self, container_ptr, containerClassName):
        ...
        if container_ptr:
            _, _, elements_size = parseQCborContainer(dumper, container_ptr)
...
class QCborValuePrinterBase(PrinterForwarder):
    ...
    def _initFromValueData(self, valueData):
        ...
        if item_type == CborValueType.Invalid.value:
            ...
            _, elements_data_ptr, elements_size = parseQCborContainer(dumper, container_ptr)
            if elements_size == 2:
